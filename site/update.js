 ...
 * Copyright (C) 2023-2023   Massi Valeskini
 *
...
function showError(error) {
    ...
    if (document.getElementById("warning").style.display === "block") {
        document.getElementById("warning").style.bottom =
            document.getElementById("error").offsetHeight + "px";
    }
    console.log(error);
}
...
function warning() {
    ...
    document.getElementById("warning_counter").innerHTML =
        cached_json.currentWarning + 1 + "/" + cached_json.warnings.length;
    document.getElementById("warning_text").innerHTML =
        "<b>" + currentWarning.title + "</b> " + currentWarning.description;
...
function clock() {
    ...
    document.getElementById("currentTime").innerHTML =
        addZeroBefore(currentTime.getHours()) +
        ":" +
...
function addDeparture(departure) {
    ...
    if (difference < 0 || walkDuration + 0.5 * 60 > difference) {
        walkStatus = "too late";
        return false;
    } else if (walkDuration + 1 * 60 > difference) {
        walkStatus = "hurry";
    } else if (walkDuration + 2.5 * 60 > difference) {
        walkStatus = "soon";
    }
    ...
    var timeString =
        "<b>" +
        addZeroBefore(departureTime.getHours()) +
        ":" +
...
function isTimeWithinRange(currentTime, rangeStr) {
    const [startTimeStr, endTimeStr] = rangeStr
        .split("-")
        .map((str) => str.trim());
...
async function checkEvents() {
    ...
    try {
        ...
        for (const event of events) {
            ...
            const isEndingSoon = isTimeWithinRange(
                new Date(currentTime.getTime() + 30 * 60 * 1000),
                rangeStr,
            );
            const isEndedRecently = isTimeWithinRange(
                new Date(currentTime.getTime() - 90 * 60 * 1000),
                rangeStr,
            );
