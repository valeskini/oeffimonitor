...
const getData = async (cb) => {
    try {
        ...
        const promises = settings.stops.map(async (stop) => {
            ...
            let location;
            if (
                jObj2["trias:Trias"]["trias:ServiceDelivery"][
                ...
                location = [
                    ...
                    ][0]["trias:Location"]["trias:GeoPosition"][
                        "trias:Longitude"
                    ],
                    jObj2["trias:Trias"]["trias:ServiceDelivery"][
                        "trias:DeliveryPayload"
            ...
            let results =
                jObj1["trias:Trias"]["trias:ServiceDelivery"][
                    "trias:DeliveryPayload"
...
const flatten = async (services, cb) => {
    ...
    await services.map(async (service) => {
        ...
        if (
            ...
            !!settings.filters.find((filter) => {
                ...
                if (keys.length === 2 && !!filter.stop && !!filter.line) {
                    ...
                    return (
                        filter.stop.indexOf(
                            service["trias:StopEvent"]["trias:ThisCall"][
                                "trias:CallAtStop"
                            ]["trias:StopPointName"]["trias:Text"],
                        ) > -1 &&
                        filter.line.indexOf(
        ...
        let walkDuration = getOSRM(
            service["trias:StopEvent"]["trias:GeoPosition"],
        );
        ...
        data.push({
            stop: service["trias:StopEvent"]["trias:ThisCall"][
                "trias:CallAtStop"
            ]["trias:StopPointName"]["trias:Text"],
            coordinates: service["trias:StopEvent"]["trias:GeoPosition"],
            line: service["trias:StopEvent"]["trias:Service"][
                "trias:ServiceSection"
            ]["trias:PublishedLineName"]["trias:Text"],
            type: service["trias:StopEvent"]["trias:Service"][
                "trias:ServiceSection"
    ...
    await axios
        .get(
            "https://verbundlinie.at/de/fahrplan/rund-um-den-fahrplan/verkehrsmeldungen",
        )
        .then((response) => {
            ...
            reports.each((i, report) => {
                ...
                if (title === "Stadt- und Umlandverkehr Graz (Zone 101)") {
                    ...
                    messages.each((j, message) => {
                        const lines = $(message)
                            .find(".linverz_lnr_stoerung")
                            .text()
                            .trim();
                        const description = $(message)
                            .find(".linverz_teilstrecke > strong")
                            .text()
                            .trim();
    ...
    trafficInfos.push({
        title: "DEMO",
        description:
            "Der Gehweg wird zu Demonstrationszwecken vom Moser am Eisernen Tor berechnet!",
    });
